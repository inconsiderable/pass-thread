...
// A small tool to inspect the pass thread and ledger offline
func main() {
	var commands = []string{
		"height", "imbalance", "imbalance_at", "pass", "pass_at", "tx", "history", "verify",
	}

	dataDirPtr := flag.String("datadir", "", "Path to a directory containing pass thread data")
	pubKeyPtr := flag.String("pubkey", "", "Base64 encoded public key")
	cmdPtr := flag.String("command", "height", "Commands: "+strings.Join(commands, ", "))
	heightPtr := flag.Int("height", 0, "Pass thread height")
	...
	switch *cmdPtr {
	case "height":
		log.Printf("Current pass thread height is: %d\n", aurora.Bold(currentHeight))
