...
// Weaver tries to weave a new tip pass.
type Weaver struct {
	pubKeys        []ed25519.PublicKey // champions of any passpoints we weave
	memo           string              // memo for passpoint of any passes we weave
...
// HashrateMonitor collects hash counts from all weavers in order to monitor and display the aggregate hashrate.
...
// NewWeaver returns a new Weaver instance.
func NewWeaver(pubKeys []ed25519.PublicKey, memo string,
	passStore PassStorage, txQueue ConsiderationQueue,
	ledger Ledger, processor *Processor,
	hashUpdateChan chan int64, num int) *Weaver {
	return &Weaver{
		pubKeys:        pubKeys,
		memo:           memo,
		passStore:     passStore,
...
// Run executes the weaver's main loop in its own goroutine.
func (m *Weaver) Run() {
	m.wg.Add(1)
	go m.run()
}
...
func (m *Weaver) run() {
	...
	// don't start weaving until we think we're synced.
	...
	if ibd {
		log.Printf("Weaver %d waiting for passthread sync\n", m.num)
	ready:
		for {
			select {
			case _, ok := <-m.shutdownChan:
				if !ok {
					log.Printf("Weaver %d shutting down...\n", m.num)
					return
				}
			case <-ticker.C:
				var err error
	...
	for {
		...
		case tip := <-tipChangeChan:
			...
			log.Printf("Weaver %d received notice of new tip pass %s\n", m.num, tip.PassID)
		...
		case newTx := <-newTxChan:
			log.Printf("Weaver %d received notice of new consideration %s\n", m.num, newTx.ConsiderationID)
			if pass == nil {
		...
		default:
			if pass == nil {
				...
				tipID, tipHeader, _, err := getThreadTipHeader(m.ledger, m.passStore)
				if err != nil {
					panic(err)
				}
				// create a new pass
			...
			if idInt.Cmp(targetInt) <= 0 {
				...
				log.Printf("Weaver %d weaved new pass %s\n", m.num, *id)

				// process the pass
				if err := m.processor.ProcessPass(*id, pass, "localhost"); err != nil {
					log.Printf("Error processing weaved pass: %s\n", err)
				}
...
// Shutdown stops the weaver synchronously.
func (m *Weaver) Shutdown() {
	close(m.shutdownChan)
	m.wg.Wait()
	log.Printf("Weaver %d shutdown\n", m.num)
}

// Create a new pass off of the given tip pass.
func (m *Weaver) createNextPass(tipID PassID, tipHeader *PassHeader) (*Pass, error) {
	log.Printf("Weaver %d weaving new pass from current tip %s\n", m.num, tipID)
	pubKey := m.pubKeys[m.keyIndex]
	return createNextPass(tipID, tipHeader, m.txQueue, m.passStore, m.ledger, pubKey, m.memo)
}

// Called by the weaver as well as the peer to support get_work.
func createNextPass(tipID PassID, tipHeader *PassHeader, txQueue ConsiderationQueue,
	...
	// create the pass
	pass, err := NewPass(tipID, newHeight, newTarget, tipHeader.ThreadWork, txs)
	if err != nil {
		return nil, err
	}
	return pass, nil
