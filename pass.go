...
// Pass represents a pass in the pass thread. It has a header and a list of considerations.
...
type Pass struct {
	...
	hasher       hash.Hash      // hash state used by weaver. not marshaled
}

// PassHeader contains data used to determine pass validity and its place in the pass thread.
type PassHeader struct {
	...
	ThreadWork        PassID            `json:"thread_work"` // total cumulative thread work
	Nonce            int64              `json:"nonce"`      // not used for crypto
	Height           int64              `json:"height"`
	ConsiderationCount int32              `json:"consideration_count"`
	hasher           *PassHeaderHasher // used to speed up weaving. not marshaled
}

// PassID is a pass's unique identifier.
type PassID [32]byte // SHA3-256 hash

// NewPass creates and returns a new Pass to be weaved.
func NewPass(previous PassID, height int64, target, threadWork PassID, considerations []*Consideration) (
	*Pass, error) {
	...
	return &Pass{
		...
		hasher:       hasher, // save this to use while weaving
	}, nil
}
...
// AddConsideration adds a new consideration to the pass. Called by weaver when weaving a new pass.
...
func addPasspointToHashListRoot(hasher hash.Hash, passpoint *Consideration) (ConsiderationID, error) {
	...
	// this made adding new considerations while weaving more efficient in a financial context
	id, err := passpoint.ID()
	if err != nil {
		return ConsiderationID{}, err
	}
...
// Compute cumulative thread work given a pass's target and the previous thread work
func computeThreadWork(target, threadWork PassID) (newThreadWork PassID) {
	passWorkInt := computePassWork(target)
	threadWorkInt := threadWork.GetBigInt()
	threadWorkInt = threadWorkInt.Add(threadWorkInt, passWorkInt)
...
// IDFast computes an ID for a given pass header when weaving.
func (header *PassHeader) IDFast(weaverNum int) (*big.Int, int64) {
	if header.hasher == nil {
		header.hasher = NewPassHeaderHasher()
	}
	return header.hasher.Update(weaverNum, header)
}
