	...
	for _, q := range m.Question {
		...
		case dns.TypeA:
			...
			for i, addr := range addresses {
				...
				if port != strconv.Itoa(DEFAULT_PASSTHREAD_PORT) {
					continue
				}
...
func (d *DNSSeeder) run() {
	...
	dns.HandleFunc("passthread.", handleDnsRequest)
...
func dnsQueryForPeers() ([]string, error) {
	...
	for _, seeder := range Seeders {
		c := dns.Client{}
		m := dns.Msg{}
		m.SetQuestion("client.passthread.", dns.TypeA)
		...
		for _, answer := range r.Answer {
			...
			peers = append(peers, a.A.String()+":"+strconv.Itoa(DEFAULT_PASSTHREAD_PORT))
		}
	}
	return peers, nil
}
