...
func main() {
	...
	DefaultPeer := "127.0.0.1:" + strconv.Itoa(DEFAULT_PASSTHREAD_PORT)
	...
	keyholder.SetConsiderationCallback(func(tx *Consideration) {
		ok, err := considerationIsRelevant(keyholder, tx)
		if err != nil {
			fmt.Printf("Error: %s\n", err)
			return
		}
	...
	keyholder.SetFilterPassCallback(func(fb *FilterPassMessage) {
	...
	for {
		...
		case "send":
			...
			id, err := sendConsideration(keyholder)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
		...
		case "txstatus":
			...
			fmt.Println("")
			tx, _, height, err := keyholder.GetConsideration(txID)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			if tx == nil {
				fmt.Printf("Consideration %s not found in the passthread at this time.\n",
					txID)
				fmt.Println("It may be waiting for confirmation.")
				break
			}
			showConsideration(keyholder, tx, height)
		...
		case "show":
			...
			if tx != nil {
				showConsideration(keyholder, tx, 0)
				if left > 0 {
		...
		case "conf":
			...
			if tx != nil {
				showConsideration(keyholder, tx.tx, tx.height)
				if left > 0 {
		...
		gpkt:
			for i, pubKey := range pubKeys {
				...
				for {
					_, stopHeight, stopIndex, fbs, err := keyholder.GetPublicKeyConsiderations(
...
// Prompt for consideration details and request the keyholder to send it
func sendConsideration(keyholder *Keyholder) (ConsiderationID, error) {
	...
	// create and send send it. by default the consideration expires if not weaved within 3 passes from now
...
func showConsideration(w *Keyholder, tx *Consideration, height int64) {
	...
	if height <= 0 {
		if tx.Matures > 0 {
			fmt.Printf("%7v: cannot be weaved until height: %d, current height: %d\n",
				aurora.Bold("Matures"), tx.Matures, header.Height)
		}
		if tx.Expires > 0 {
			fmt.Printf("%7v: cannot be weaved after height: %d, current height: %d\n",
...
func considerationIsRelevant(keyholder *Keyholder, tx *Consideration) (bool, error) {
	pubKeys, err := keyholder.GetKeys()
	if err != nil {
		return false, err
	}
