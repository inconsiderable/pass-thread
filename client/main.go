...
import (
	...
	. "github.com/inconsiderable/pass-thread"
	"golang.org/x/crypto/ed25519"
)

// A peer node in the passthread network
func main() {
	rand.Seed(time.Now().UnixNano())

	// flags
	pubKeyPtr := flag.String("pubkey", "", "A public key which receives newly weaved pass points")
	dataDirPtr := flag.String("datadir", "", "Path to a directory to save pass thread data")
	memoPtr := flag.String("memo", "", "A memo to include in newly weaved passes")
	portPtr := flag.Int("port", DEFAULT_PASSTHREAD_PORT, "Port to listen for incoming peer connections")
	peerPtr := flag.String("peer", "", "Address of a peer to connect to")
	upnpPtr := flag.Bool("upnp", false, "Attempt to forward the passthread port on your router with UPnP")
	...
	numWeaversPtr := flag.Int("numweavers", 1, "Number of weavers to run")
	...
	keyFilePtr := flag.String("keyfile", "", "Path to a file containing public keys to use when weaving")
	genPassPtr := flag.String("genpass", "", "Path to a json file containing the genesis pass for the thread")
	...
	if len(*peerPtr) != 0 {
		...
		if i := strings.LastIndex(*peerPtr, ":"); i < 0 {
			*peerPtr = *peerPtr + ":" + strconv.Itoa(DEFAULT_PASSTHREAD_PORT)
		}
	}
	...
	// load public keys to weave to
	var pubKeys []ed25519.PublicKey
	if *numWeaversPtr > 0 {
		if len(*pubKeyPtr) == 0 && len(*keyFilePtr) == 0 {
			log.Fatal("-pubkey or -keyfile argument required to receive newly weaved pass points")
		}
		if len(*pubKeyPtr) != 0 && len(*keyFilePtr) != 0 {
	...
	if *numWeaversPtr > 0 {
		hashUpdateChan := make(chan int64, *numWeaversPtr)
		// create and run weavers
		for i := 0; i < *numWeaversPtr; i++ {
			weaver := NewWeaver(pubKeys, *memoPtr, passStore, txQueue, ledger, processor, hashUpdateChan, i)
			weavers = append(weavers, weaver)
			weaver.Run()
		}
		...
		log.Println("Weaving is currently disabled")
	}
