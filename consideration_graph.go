...
func (g *Graph) ToDOT(pubKey string, catchments []string) string {	
	...
	pkIndex := g.index[pubKey] //defaults to zero- the passpoint
	...
	for _, id := range includedNodes {	
		...
		if ok, _plusCode, _catchment, _ := plusCodeFromIdentifier(node.pubkey, catchments); ok {
			...
			if okk, _, room, fixture, issues := inflateConsiderationFrontier(node.pubkey); okk {						
...
// Rank computes the ConsiderabilityRank of every node in the directed graph.
