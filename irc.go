...
// port is our local passthread port. If it's set to 0 we won't be used for inbound connections.
func (i *IRC) Connect(genesisID PassID, port int, addrChan chan<- string) error {
...
func generateChannelName(genesisID PassID, n int) string {
	g := genesisID.String()
	return "#passthread-" + g[len(g)-8:] + "-" + strconv.Itoa(n)
}
