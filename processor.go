...
// It also manages the storage of all pass thread data as well as inclusion of new considerations into the consideration queue.
type Processor struct {
	...
	tipChangeChannels       map[chan<- TipChange]struct{} // channels needing notification of changes to main thread tip passes
...
type TipChange struct {
	PassID PassID   // pass ID of the main thread tip pass
	Pass   *Pass    // full pass
...
// ProcessPass is called to process a new candidate pass thread tip.
...
// Attempt to extend the thread with the new pass
func (p *Processor) acceptPass(id PassID, pass *Pass, now int64, source string) error {
	...
	threadWork := computeThreadWork(pass.Header.Target, prevHeader.ThreadWork)
	if pass.Header.ThreadWork != threadWork {
		return fmt.Errorf("Incorrect thread work %s, expected %s for pass %s",
			pass.Header.ThreadWork, threadWork, id)
	}
...
func (p *Processor) acceptPassContinue(
	...
	if !pass.Header.Compare(tipHeader, passWhen, tipWhen) {
		...
		log.Printf("Pass %s does not represent the tip of the best thread", id)
		return p.ledger.SetBranchType(id, SIDE)
	}

	// the new pass is the better thread
	...
	// we're at common ancestor. disconnect any main thread passes we need to
	...
	// connect any new thread passes we need to
...
func getThreadTipHeader(ledger Ledger, passStore PassStorage) (*PassID, *PassHeader, int64, error) {
	// get the current tip
