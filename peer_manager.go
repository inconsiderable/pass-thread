...
func NewPeerManager(
	genesisID PassID, peerStore PeerStorage, passStore PassStorage,
	ledger Ledger, processor *Processor, indexer *Indexer, txQueue ConsiderationQueue,
...
func (p *PeerManager) connectToPeers(ctx context.Context) error {
	...
	if ibd {
		...
		// thread as soon as we think we're done with them and find more peers
		want = 1
	} else {
...
func (p *PeerManager) connect(ctx context.Context, addr string) (int, *Peer, error) {
	peer := NewPeer(nil, p.genesisID, p.peerStore, p.passStore, p.ledger, p.processor, p.indexer, p.txQueue, p.passQueue, p.addrChan)
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		theirNonce := r.Header.Get("Passthread-Peer-Nonce")
		...
		theirAddress := r.Header.Get("Passthread-Peer-Address")
		...
		peer := NewPeer(conn, p.genesisID, p.peerStore, p.passStore, p.ledger, p.processor, p.indexer, p.txQueue, p.passQueue, p.addrChan)
	...
	// listen for websocket requests using the genesis pass ID as the handler pattern
...
// IsInitialPassDownload returns true if it appears we're still syncing the pass thread.
func IsInitialPassDownload(ledger Ledger, passStore PassStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getThreadTipHeader(ledger, passStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
