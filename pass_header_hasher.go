...
// PassHeaderHasher is used to more efficiently hash JSON serialized pass headers while weaving.
...
func NewPassHeaderHasher() *PassHeaderHasher {
	...
	bufLen += len(hdrThreadWork) + len(hdrNonce) + len(hdrHeight) + len(hdrConsiderationCount)
...
func (h *PassHeaderHasher) Update(weaverNum int, header *PassHeader) (*big.Int, int64) {
	var deviceWeaving bool = false
	...
	if !h.initialized {
		...
		// nonce
		if offset != 0 || (!deviceWeaving && h.previousNonce != header.Nonce) {
			//bufferChanged = true
			h.previousNonce = header.Nonce
	...
	// 	nonce := h.updateDevice(weaverNum, header, bufferChanged)
	// 	if nonce == 0x7FFFFFFFFFFFFFFF {
	// 		// not found
	// 		h.result.SetBytes(
	// 			// indirectly let weaver.go know we failed
	...
	// 	} else {
	// 		log.Printf("GPU weaver %d found a possible solution: %d, double-checking it...\n",
	// 			weaverNum, nonce)
...
// Handle weaving with GPU devices
// func (h *PassHeaderHasher) updateDevice(weaverNum int, header *PassHeader, bufferChanged bool) int64 {
// 	if bufferChanged {
// 		// update the device's copy of the buffer
// 		lastOffset := h.nonceOffset + h.nonceLen
// 		if CUDA_ENABLED {
// 			h.hashesPerAttempt = CudaWeaverUpdate(weaverNum, h.buffer, h.bufLen,
// 				h.nonceOffset, lastOffset, header.Target)
// 		} else {
// 			h.hashesPerAttempt = OpenCLWeaverUpdate(weaverNum, h.buffer, h.bufLen,
// 				h.nonceOffset, lastOffset, header.Target)
// 		}
// 	}
// 	// try for a solution
// 	var nonce int64
// 	if CUDA_ENABLED {
// 		nonce = CudaWeaverWeave(weaverNum, header.Nonce)
// 	} else {
// 		nonce = OpenCLWeaverWeave(weaverNum, header.Nonce)
// 	}
// 	return nonce
// }
